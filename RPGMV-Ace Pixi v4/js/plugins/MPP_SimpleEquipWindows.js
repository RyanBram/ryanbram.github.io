...
(() => {
    ...
    const module_CursorLink = (obj) => {
        ...
        obj.addCursorLinkWindow = function (window) {
            if (!this._cursorLinkWindows.includes(window) && window._cursorLinkWindows) {
        ...
        obj.isCancelEnabled = function () {
            return _base_isCancelEnabled.apply(this, arguments) && this.index() >= 0;
        };
...
//        this.addCommand(TextManager.equip2, "equip");
    ...
    const _Window_EquipCommand_processOk = Window_EquipCommand.prototype.processOk;
    Window_EquipCommand.prototype.processOk = function () {
        if (this.currentSymbol() === "equip") {
            // Jika command adalah 'equip', mainkan suara dan panggil handler-nya
            // secara langsung, lewati proses deactivate() dari induk.
            this.playOkSound();
            this.updateInputData();
            this.callHandler("equip");
        } else {
            // Untuk command lain ('optimize', 'clear'), jalankan fungsi aslinya.
            _Window_EquipCommand_processOk.call(this);
        }
    };
    ...
    Scene_Equip.prototype.createCommandWindow = function () {
        ...
        this._commandWindow.setHandler("equip", this.commandDown.bind(this));
