   ...
   //=============================================================================
    ...
    Graphics._setPointerTypeCustom = function() {
        if (cursorIcon === 'custom') {
            ...
            this._PointerType = `url('data:image/png;base64,${cursorBase64}'), default`;
        }
    };
...
if (function() {
    ...
    // Inserting plugin file name as parameters
    var parameters = PluginManager.parameters(currentFileName);
    var enabled = parameters['vorbisStreaming'] === 'true';
    var measured = parameters['audioMeasurement'] === 'true';
    ...
    if (measured) {
        ...
        SceneManager.updateManagers = function() {
            ...
            var _WebAudio__readLoopComments =
...
if (window.ResourceHandler) {
    ResourceHandler.fetchWithRetry = async function(
        ...
        let retry;
        try {
            var response = await (!window.cordova ?
        ...
        if (!retry) {
            var error = new Error('Failed to load: ' + url);
...
WebAudio.prototype._load = async function(url) {
    if (WebAudio._context) {
        ...
        var reader = await ResourceHandler.fetchWithRetry('stream', url);
...
WebAudio.prototype._loading = async function(reader) {
    try {
        var decode = stbvorbis.decodeStream(result => this._onDecode(result));
        while (true) {
...
WebAudio.prototype._onDecode = function(result) {
    ...
    var chunk = { buffer, sourceNode: null, when: this._loadedTime };
...
WebAudio.prototype._calcSourceNodeParams = function(chunk) {
    ...
    var chunkEnd = chunk.when + chunk.buffer.duration;
    var pos = this.seek();
    let when, offset, duration;
    if (this._loop && this._loopLength) {
        var loopEnd = this._loopStart + this._loopLength;
        if (pos <= chunk.when) {
...
WebAudio.prototype._createSourceNode = function(chunk) {
    ...
    var params = this._calcSourceNodeParams(chunk);
    if (!params) {
    ...
    var { when, offset, duration } = params;
