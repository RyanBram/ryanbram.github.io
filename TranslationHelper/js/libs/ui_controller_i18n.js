...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // --- Mode Detection ---
    const modeInfo = i18nEngine.getModeInfo();
    console.log("[UI Controller] Mode Info:", modeInfo);

    // --- Bergamot Translator (only load in server mode) ---
    let BatchTranslator = null;
    let TranslatorBacking = null;

    if (modeInfo.translationAvailable) {
        try {
            console.log("[UI Controller] Loading Bergamot translator module...");
            const mod = await import("../../node_modules/@browsermt/bergamot-translator/translator.js");
            BatchTranslator = mod.BatchTranslator;
            TranslatorBacking = mod.TranslatorBacking;
            console.log("[UI Controller] Bergamot module loaded successfully");
        } catch (error) {
            console.error("[UI Controller] Failed to load Bergamot module:", error);
            console.warn("[UI Controller] Translation features will be disabled");
            // Don't show error to user - translation_engine.js will handle it
        }
    } else {
        console.warn("[UI Controller] Translation features disabled - running in LOCAL mode");
    }
    ...
    // ============================================
    // Mode Warning & Feature Detection
    // ============================================
    ...
    function showModeWarning() {
        ...
        if (header) {
            ...
            modeIndicator.style.cssText =
                "margin-left: 15px; font-size: 0.7em; color: #ff6b6b; background: #fff3cd; padding: 4px 12px; border-radius: 4px; font-weight: bold;";
            modeIndicator.textContent = "⚠️ LOCAL MODE - Translation Disabled";
            modeIndicator.title =
                "Running without server. Translation features are unavailable.\nBasic features: Upload, Extract, Copy, Paste, Rebuild JSON";
            header.appendChild(modeIndicator);
        }

        // Update build status
        if (ui.buildStatus) {
            ui.buildStatus.textContent = "⚠️ Local mode: Translation disabled. Other features available.";
            ui.buildStatus.style.color = "#856404";
        }
    }

    function disableTranslationFeatures() {
        // Disable translate buttons
        ui.translateAllBtn.disabled = true;
        ui.translateAllBtn.title = "Translation not available in local mode. Please run with a web server.";
        ui.translateAllBtn.style.opacity = "0.5";

        // Update button text
        ui.translateAllBtn.textContent = "🚫 Translate (Server Required)";
    }

    function enableTranslationFeatures() {
        // Enable translate buttons
        ui.translateAllBtn.disabled = false;
        ui.translateAllBtn.title = "Translate all text automatically";
        ui.translateAllBtn.style.opacity = "1";
    ...
    function downloadPlugin() {
        // Create blob from rpg_locale.js file
        fetch("js/plugins/rpg_locale.js")
            .then((response) => {
                if (!response.ok) {
            ...
            .catch((error) => {
                ...
                alert(
                    "❌ Plugin file not found!\n\n" +
                        "Please ensure rpg_locale.js is in the same folder as this HTML file.\n\n" +
    ...
    function generateTranslationColumns() {
        ...
        selectedLanguages.forEach((langItem) => {
            ...
            // Check if translation is available for button state
            const translateDisabled = !modeInfo.translationAvailable;
            const translateTitle = translateDisabled
                ? "Translation not available in local mode"
                : "Translate this language automatically";

            header.innerHTML = `
                <h2>
                    <img src="${lang.flag}" alt="${
                lang.code
            }" style="width: 20px; height: 15px; margin-right: 5px; vertical-align: middle; border-radius: 2px;">${
                lang.name
            }
                    <span class="translation-status" id="status-${langCode}" style="margin-left: 8px; display: none;">✅</span>
                </h2>
                <button class="util-btn translate-btn" data-lang="${langCode}" ${
                translateDisabled ? "disabled" : ""
            } title="${translateTitle}" style="${translateDisabled ? "opacity: 0.5;" : ""}">
                    ${translateDisabled ? "🚫 Translate" : "Translate"}
                </button>
    ...
    function populateTranslationsFromI18n() {
        ...
        if (compatibilityIssues > 0) {
            console.warn(`⚠️ i18n compatibility issues: ${compatibilityIssues} texts not found in existing i18n data`);
    ...
    function buildI18nMapping() {
        ...
        selectedLanguages.forEach((langItem) => {
            ...
            if (actualTranslatedLines !== expectedLineCount) {
                ...
                errorMessages.push(`${lang.name}: Expected ${expectedLineCount} lines, got ${actualTranslatedLines}`);
            } else {
        ...
        if (!modeInfo.translationAvailable) {
            alert(
                "⚠️ TRANSLATION NOT AVAILABLE\n\n" +
                    "You are running in LOCAL mode (file://).\n\n" +
                    "Translation features require:\n" +
                    "• A web server (http://localhost or https://)\n" +
                    "• OR NW.js application mode\n\n" +
                    "AVAILABLE FEATURES IN LOCAL MODE:\n" +
                    "✅ Upload JSON files\n" +
                    "✅ Extract text\n" +
                    "✅ Copy extracted text\n" +
                    "✅ Paste translations (from external tools)\n" +
                    "✅ Rebuild i18n JSON\n\n" +
                    "To enable translation:\n" +
                    "1. Double click Run Server Mode.bat\n" +
                    "2. Wait until browser window opened  http://localhost:8000\n" +
                    "3. Translate All and Translate button will become active"
            );
            return;
        }
    ...
    async function translateIndividualText(langCode) {
        ...
        if (!modeInfo.translationAvailable) {
            alert(
                "⚠️ TRANSLATION NOT AVAILABLE\n\n" +
                    "You are running in LOCAL mode (file://).\n\n" +
                    "Please use external translation tools:\n" +
                    '1. Click "Copy" to copy extracted text\n' +
                    "2. Use Google Translate, DeepL, or ChatGPT\n" +
                    '3. Click "Paste" to paste translated text\n\n' +
                    "Or run with a web server to enable automatic translation."
            );
            return;
        }
        ...
        try {
            ...
            // Create new translator backing and setup for individual translation
            const translatorBacking = new TranslatorBacking({ registryUrl: "models/registry.json" });
            ...
            if (typeof nw !== "undefined") {
                ...
                if (fsPromises && pathModule && urlModule) {
                    try {
                        ...
                        console.error("NW.js: Gagal memuat registry untuk translateIndividual:", err);
                    }
                }
            } else if (typeof window !== "undefined") {
                try {
                    ...
                    console.error("Browser: Gagal memuat registry untuk translateIndividual:", err);
                }
            }
    ...
    if (!modeInfo.translationAvailable) {
        showModeWarning();
        disableTranslationFeatures();
    } else {
        enableTranslationFeatures();
    }
