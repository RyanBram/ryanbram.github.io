...
class TranslationEngineI18n {
    ...
    _protectEscapeCodes(text) {
        ...
        singleLineText = singleLineText.replace(escapeRegex, "{{PRTK$1}}");

        // 3. CRITICAL: Protect format placeholders %1, %2, %3, etc (RMMV message format)
        //    These are used in System.json messages like "%1 took %2 damage!"
        //    Using {{FRMT}} format - Bergamot won't translate short abbreviations without spaces
        const formatRegex = /(%\d+)/g;
        singleLineText = singleLineText.replace(formatRegex, (match, num) => {
            // Convert %1 → {{FRMT1}}, %2 → {{FRMT2}}, etc
            return `{{FRMT${num.substring(1)}}}`;
        });
    ...
    _restoreEscapeCodes(text) {
        // 1. Restore escape codes: {{PRTK\N[1]}} → \N[1]
        const restoreRegex = /\{\{PRTK(\\[VvNnPpGgCcIi]\[\d+\]|\\[$.|\!><{}^\\])\}\}/g;
        text = text.replace(restoreRegex, "$1");

        // 2. Restore format placeholders: {{FRMT1}} → %1, {{FRMT2}} → %2, etc
        const formatRestoreRegex = /\{\{FRMT(\d+)\}\}/g;
        text = text.replace(formatRestoreRegex, "%$1");

        // 3. Restore newlines: {{NWLN}} → \n
        text = text.replace(/\{\{NWLN\}\}/g, "\n");
        
        // 4. CRITICAL: Clean up incomplete patterns from translation corruption
        //    Bergamot sometimes corrupts patterns, leaving incomplete {{PRTK\i[90] at end
        if (text.includes("{{PRTK")) {
            const beforeCleanup = text;
            
            // Remove incomplete PRTK pattern at end of string
            text = text.replace(/\{\{PRTK\\[VvNnPpGgCcIi]\[\d+\]$/g, "");
            
            if (beforeCleanup !== text) {
                console.warn("[_restoreEscapeCodes] WARNING: Removed incomplete PRTK pattern at end");
                console.warn("  Before:", beforeCleanup.substring(Math.max(0, beforeCleanup.length - 100)));
                console.warn("  After:", text.substring(Math.max(0, text.length - 100)));
            }
        }
    ...
    _safeEventListExtractor(jsonData, isDatabaseType) {
        ...
        items.forEach((item, itemIndex) => {
            ...
            // ========================================================================
            // FIX: Deteksi apakah item memiliki "pages" (Troop) atau "list" (CommonEvent)
            // ========================================================================
            const hasPages = item.pages && Array.isArray(item.pages);
            const hasList = item.list && Array.isArray(item.list);

            if (hasPages) {
                // TROOP EVENT: Memiliki pages
                item.pages.forEach((page, pageIndex) => {
                    if (page && Array.isArray(page.list)) {
                        page.list.forEach((command, commandIndex) => {
                            // Handle Show Text (401, 402, 405)
                ...
                // COMMON EVENT: Tidak memiliki pages, langsung memiliki list
                item.list.forEach((command, commandIndex) => {
                    // Handle Show Text (401, 402, 405)
                    if (command && (command.code === 401 || command.code === 402 || command.code === 405)) {
                        const pIndex = command.code === 401 || command.code === 405 ? 0 : 1;
                        const text = command.parameters?.[pIndex];
    ...
    _safeMapEventListExtractor(jsonData) {
        ...
        items.forEach((item, itemIndex) => {
            ...
            // Map events ALWAYS have pages (not like Common Events)
    ...
    buildI18nMapping(textPaths, translatedTexts, targetLanguage) {
        ...
        textPaths.forEach((pathInfo, index) => {
            ...
            // CRITICAL FIX: Restore escape codes before storing
            // This ensures {{PRTK\c[17]}} → \c[17] and {{FRMT1}} → %1
            const restoredText = this._restoreEscapeCodes(translatedText);

            // Build nested structure from path
            // path example: ["1", "name"] → mapping["1"]["name"] = "Harold"
            this._setNestedValue(mapping, pathInfo.path, restoredText);
        });
