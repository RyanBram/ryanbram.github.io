...
(function () {
   ...
   var paramDefaultLanguageNames = {
    ...
    var paramLanguageNames = paramAvailableLanguages.map(code => paramDefaultLanguageNames[code] || code);
    ...
    window.LanguageSwitcher = {
        ...
        _restoreEscapeCodes: function (text) {
            ...
            if (text.includes("{{PRTK")) {
                ...
                if (beforeCleanup !== text) {
                    console.warn("[_restoreEscapeCodes] ⚠️ Removed incomplete PRTK pattern at end of string");
            ...
            if (originalText !== text && (originalText.includes("{{FRMT") || originalText.includes("{{PRTK"))) {
        ...
        getTranslation: function (dataType, path, fallback) {
            ...
            if (!data) {
                ...
                console.warn(`[getTranslation] Language ${lang} not found in ${dataType}`);
            ...
            try {
                for (const key of path) {
                    if (value === undefined || value === null) {
                        ...
                        console.warn(`[getTranslation] Path not found: ${dataType}.${lang}.${path.join(".")}`);
                        return fallback;
                    }
                    value = value[key];
                }
                ...
                console.error(`[getTranslation] Error: ${dataType}.${lang}.${path.join(".")}`, e);
                return fallback;
            }
            ...
            if (typeof value === "string") {
                ...
                if (beforeRestore.includes("{{PRTK") || beforeRestore.includes("{{FRMT")) {
            ...
            if (value !== fallback) {
                console.log(`[getTranslation] ✓ ${dataType}.${lang}.${path.join(".")} = "${value}"`);
            }
    ...
    Game_System.prototype.initialize = function () {
        ...
        const savedLanguage = ConfigManager.language || LanguageSwitcher.currentLanguage || paramDefaultLanguage;
    ...
    Window_Status.prototype.drawProfile = function (x, y) {
        ...
        if (paramPreserveManualLineBreaks) {
            ...
            wrappedProfile = LanguageSwitcher._wrapSingleLine(continuousText, maxWidth, this);
        }
    ...
    Window_Help.prototype.setItem = function (item) {
        ...
        if (paramPreserveManualLineBreaks) {
            ...
            wrappedDesc = LanguageSwitcher._wrapSingleLine(continuousText, maxWidth, this);
        }
    ...
    function getEventCommandPath(interpreter, commandIndex, paramIndex) {
        ...
        if ($dataCommonEvents && interpreter._list) {
            for (let i = 1; i < $dataCommonEvents.length; i++) {
                if ($dataCommonEvents[i] && $dataCommonEvents[i].list === interpreter._list) {
                    isCommonEvent = true;
                    commonEventId = i;
                    break;
                }
        ...
        } else if ($gameTroop && $gameTroop._troopId > 0 && $gameTroop._enemies.length > 0) {
            // ==================
            // 2. TROOP EVENT
            // ==================
            dataType = "Troops";
    ...
    Window_Options.prototype.changeLanguage = function (direction) {
        ...
        const currentLang = $gameSystem ? $gameSystem.getLanguage() : (ConfigManager.language || LanguageSwitcher.currentLanguage || paramDefaultLanguage);
        ...
        ConfigManager.save();
        console.log("[Options] Language changed to:", newLang, "and saved to ConfigManager");
