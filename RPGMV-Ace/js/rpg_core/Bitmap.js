...
Bitmap.snap = function (stage) {
  ...
  var renderTexture = PIXI.RenderTexture.create({
    width: width,
    height: height,
  }); // PERUBAHAN 1: .create() sekarang menerima objek
  if (stage) {
    Graphics._renderer.render(stage, { renderTexture: renderTexture }); // PERUBAHAN 2: .render() sekarang menerima objek untuk renderTexture
    stage.worldTransform.identity();
    var canvas = null;
    if (Graphics.isWebGL()) {
      canvas = Graphics._renderer.extract.canvas(renderTexture);
    } else {
      canvas = Graphics._renderer.extract.canvas(renderTexture); // PERUBAHAN 3: Metode ekstraksi kanvas Metode di atas (extract.canvas) sekarang berfungsi untuk WebGL dan Canvas
    }
 ...
 * @param {String} align The alignment of the text
 */
Bitmap.prototype.drawSmallText = function (
  text,
  x,
  y,
  maxWidth,
