...
var BHell = (function (my) {
    ...
    var parameters = PluginManager.parameters('Danmaku_core');
    ...
    Scene_Boot.prototype.create = function () {
        ...
        DataManager._databaseFiles.push({name: "$dataEmitters", src: "Emitters.json"});
        _Boot_create.call(this);
    };

    // Helper function to expand emitter group IDs to actual emitter arrays
    my.expandEmitterGroup = function(groupId) {
        if (!$dataEmitters || !$dataEmitters.groups) {
            console.warn("BulletHell: Emitters.json groups not loaded!");
            return [groupId];
        }
        return $dataEmitters.groups[groupId] || [groupId]; // If not a group, return as single-element array
    };

    // Helper function to get emitter by ID
    my.getEmitterById = function(id) {
        if (!$dataEmitters || !$dataEmitters.emitters) {
            console.warn("BulletHell: Emitters.json not loaded!");
            return null;
        }
        var emitter = $dataEmitters.emitters.find(function(e) {
            return e.id === id;
        });
        if (!emitter) {
            console.warn("BulletHell: Emitter ID " + id + " not found!");
        }
        return emitter;
    };

    // Helper function to parse Item notetags for BulletHell actor unlock
    my.parseItemNotetag = function(item) {
        if (!item || !item.note) return null;
        
        // Parse <bhell_actor:actorId> tag (new syntax)
        var bhellActor = /<bhell_actor:\s*(\d+)>/i.exec(item.note);
        if (bhellActor) {
            return parseInt(bhellActor[1]);
        }
        
        // Also support old syntax <bhell_type:actorId> for backward compatibility
        var bhellType = /<bhell_type:\s*(\d+)>/i.exec(item.note);
        if (bhellType) {
            return parseInt(bhellType[1]);
        }
        
        return null;
    };

    // Helper function to parse Actor notetags
    my.parseActorNotetag = function(actor) {
        if (!actor || !actor.note) return {};
        
        var result = {};
        var note = actor.note;
        
        // Parse hitbox
        var hitboxW = /<bhell:hitbox_w:\s*(.+?)>/i.exec(note);
        if (hitboxW) result.hitbox_w = hitboxW[1].trim();
        
        var hitboxH = /<bhell:hitbox_h:\s*(.+?)>/i.exec(note);
        if (hitboxH) result.hitbox_h = hitboxH[1].trim();
        
        // Parse sound effects
        var selectSe = /<bhell:select_se:\s*({[\s\S]*?})>/i.exec(note);
        if (selectSe) {
            try {
                result.select_se = JSON.parse(selectSe[1]);
            } catch (e) {
                console.warn("BulletHell: Failed to parse select_se for actor " + actor.id);
            }
        }
        
        var spawnSe = /<bhell:spawn_se:\s*({[\s\S]*?})>/i.exec(note);
        if (spawnSe) {
            try {
                result.spawn_se = JSON.parse(spawnSe[1]);
            } catch (e) {
                console.warn("BulletHell: Failed to parse spawn_se for actor " + actor.id);
            }
        }
        
        var deathSe = /<bhell:death_se:\s*({[\s\S]*?})>/i.exec(note);
        if (deathSe) {
            try {
                result.death_se = JSON.parse(deathSe[1]);
            } catch (e) {
                console.warn("BulletHell: Failed to parse death_se for actor " + actor.id);
            }
        }
        
        // Parse bomb
        var bomb = /<bhell:bomb:\s*({[\s\S]*?})>/i.exec(note);
        if (bomb) {
            try {
                result.bomb = JSON.parse(bomb[1]);
            } catch (e) {
                console.warn("BulletHell: Failed to parse bomb for actor " + actor.id);
            }
        }
        
        // Parse emitter IDs (array of integers)
        var emitters = /<bhell:emitters:\s*\[([^\]]+)\]>/i.exec(note);
        if (emitters) {
            try {
                result.emitter_ids = JSON.parse("[" + emitters[1] + "]");
            } catch (e) {
                console.warn("BulletHell: Failed to parse emitters for actor " + actor.id);
            }
        }
        
        // Parse unlock condition - NO LONGER USED
        // Unlock is now handled by items with <bhell_actor:actorId> tag
        // var unlockItem = /<bhell:unlock_item:\s*(\d+)>/i.exec(note);
        // if (unlockItem) {
        //     result.unlock_item_id = parseInt(unlockItem[1]);
        // }
        
        return result;
    };

    // Helper function to parse skill notetags for BulletHell
    my.parseSkillNotetag = function(skill) {
        ...
        if (bomb) {
            try {
                ...
                console.warn("BulletHell: Failed to parse bomb for skill " + skill.id);
            }
        }
        ...
        if (emitters) {
            try {
                ...
                console.warn("BulletHell: Failed to parse emitters for skill " + skill.id);
            }
        }
    ...
    // Helper function to get actor's BulletHell skills
    my.getActorBHellSkills = function(actorId) {
        var actor = $dataActors[actorId];
        if (!actor) return {weapons: [], bombs: []};
        
        // Get all skills learnable by this actor's class
        var actorClass = $dataClasses[actor.classId];
        if (!actorClass || !actorClass.learnings) return {weapons: [], bombs: []};
        
        var weaponSkills = [];
        var bombSkills = [];
        
    for (var i = 0; i < actorClass.learnings.length; i++) {
        var learning = actorClass.learnings[i];
        var skill = $dataSkills[learning.skillId];
        
        if (skill) {
            var skillData = my.parseSkillNotetag(skill);
            if (skillData.emitter_ids) {
                weaponSkills.push({
                    skillId: skill.id,
    ...
    }        return {weapons: weaponSkills, bombs: bombSkills};
    };

    // Helper function to map class stats to ranks
    // Since base stats start at 1, each +1 increases the rank
    my.statToRank = function(statValue) {
        if (statValue >= 5) return "S";
        if (statValue >= 4) return "A";
        if (statValue >= 3) return "B";
        if (statValue >= 2) return "C";
        return "D"; // statValue = 1
    };
    ...
    Game_Interpreter.prototype.pluginCommand = function (command, args) {
        ...
        if (command === 'Bullethell') {
            ...
            // If the players' settings aren't loaded yet, load them from the JSON or Actors database.
            $gamePlayer.bhellPlayers = $gamePlayer.bhellPlayers || [];
            
            // First, try to load from Actors database (new system)
            var actorsWithBHell = [];
            for (var a = 1; a < $dataActors.length; a++) {
                if ($dataActors[a] && $dataActors[a].note && $dataActors[a].note.indexOf("<bhell:") >= 0) {
                    actorsWithBHell.push(a);
                }
            }
            
            // If actors with BHell tags exist, use them; otherwise fallback to JSON
            if (actorsWithBHell.length > 0) {
                for (var i = 0; i < actorsWithBHell.length; i++) {
                    var actorId = actorsWithBHell[i];
                    var actor = $dataActors[actorId];
                    var actorClass = $dataClasses[actor.classId];
                    var notetag = my.parseActorNotetag(actor);
                    
                    $gamePlayer.bhellPlayers[i] = $gamePlayer.bhellPlayers[i] || {};
                    $gamePlayer.bhellPlayers[i].index = i;
                    $gamePlayer.bhellPlayers[i].actorId = actorId; // Store actor ID for reference
                    
                    // Check unlock condition based on items with <bhell_actor:actorId> tag
                    var hasUnlockItem = false;
                    for (var itemId = 1; itemId < $dataItems.length; itemId++) {
                        if ($dataItems[itemId]) {
                            var itemActorId = my.parseItemNotetag($dataItems[itemId]);
                            if (itemActorId === actorId && $gameParty.hasItem($dataItems[itemId])) {
                                hasUnlockItem = true;
                                break;
                            }
                        }
                    ...
                    // Always update unlock status based on item possession
                    $gamePlayer.bhellPlayers[i].unlocked = hasUnlockItem;
                    
                    // Stats from Classes
                    $gamePlayer.bhellPlayers[i].speed = $gamePlayer.bhellPlayers[i].speed || my.statToRank(actorClass.params[6][1]); // Agility at level 1
                    $gamePlayer.bhellPlayers[i].rate = $gamePlayer.bhellPlayers[i].rate || my.statToRank(actorClass.params[7][1]); // Luck at level 1
                    $gamePlayer.bhellPlayers[i].power = $gamePlayer.bhellPlayers[i].power || my.statToRank(actorClass.params[2][1]); // Attack at level 1
                    $gamePlayer.bhellPlayers[i].bombs = $gamePlayer.bhellPlayers[i].bombs || my.statToRank(actorClass.params[4][1]); // M.Attack at level 1
                    
                    // Load bomb and emitters from skills (priority) or notetags (fallback)
                    var bhellSkills = my.getActorBHellSkills(actorId);
                    
                    // Load weapon (emitters) from skills
                    if (bhellSkills.weapons.length > 0) {
                        var primaryWeapon = bhellSkills.weapons[0];
                        if (primaryWeapon.data.emitter_ids) {
                            $gamePlayer.bhellPlayers[i].emitterIds = primaryWeapon.data.emitter_ids;
                            $gamePlayer.bhellPlayers[i].weaponSkillId = primaryWeapon.skillId;
                        }
                    } else if (notetag.emitter_ids) {
                        // Fallback to actor notetag for emitters
                        $gamePlayer.bhellPlayers[i].emitterIds = notetag.emitter_ids;
                    }
                    
                    // Load bomb from skills  
                    if (bhellSkills.bombs.length > 0) {
                        var primaryBomb = bhellSkills.bombs[0];
                        if (primaryBomb.data.bomb) {
                            $gamePlayer.bhellPlayers[i].bombData = primaryBomb.data.bomb;
                            $gamePlayer.bhellPlayers[i].bombSkillId = primaryBomb.skillId;
                        }
                    } else if (notetag.bomb) {
                        // Fallback to actor notetag for bomb
                        $gamePlayer.bhellPlayers[i].bombData = notetag.bomb;
                    }
                    
                    // Store parsed data from notetags
                    $gamePlayer.bhellPlayers[i].notetag = notetag;
                }
            } else {
    ...
    // Hook into Game_Party.gainItem to auto-unlock actors when items with <bhell_actor:actorId> are obtained
    var _Game_Party_gainItem = Game_Party.prototype.gainItem;
    Game_Party.prototype.gainItem = function(item, amount, includeEquip) {
        _Game_Party_gainItem.call(this, item, amount, includeEquip);
        
        // Check if this is a BulletHell actor unlock item
        if (item && DataManager.isItem(item) && amount > 0) {
            var actorId = BHell.parseItemNotetag(item);
            if (actorId && $gamePlayer.bhellPlayers) {
                // Find and unlock the corresponding player
                for (var i = 0; i < $gamePlayer.bhellPlayers.length; i++) {
                    if ($gamePlayer.bhellPlayers[i] && $gamePlayer.bhellPlayers[i].actorId === actorId) {
                        $gamePlayer.bhellPlayers[i].unlocked = true;
                        console.log("BulletHell: Unlocked player for Actor ID " + actorId);
                        break;
                    }
                }
            }
...
var BHell = (function (my) {
    ...
    BHell_Player.prototype.initialize = function (id, lives, unlimitedBombs, parent) {
        // Check if player data comes from Actor database or JSON
        var playerData = null;
        var playerParams = Object.assign({}, $gamePlayer.bhellPlayers.filter(p => {
            return p.index === id;
        })[0]);
        
        var usingActorSystem = playerParams.actorId !== undefined;
        
        if (usingActorSystem) {
            // New system: Load from Actor database
            var actor = $dataActors[playerParams.actorId];
            var notetag = playerParams.notetag || {};
            
            // Build playerData from Actor
            playerData = {
                sprite: actor.characterName,
                index: actor.characterIndex,
                direction: 8, // Default upward
                frame: 0,
                animated: true,
                animation_speed: 30,
                hitbox_w: notetag.hitbox_w || "w/2",
                hitbox_h: notetag.hitbox_h || "h/2",
                grazing_radius: notetag.grazing_radius || "w",
                select_se: notetag.select_se || null,
                spawn_se: notetag.spawn_se || null,
                death_se: notetag.death_se || null,
                victory: notetag.victory || null,
                bomb: playerParams.bombData || notetag.bomb || null,
                emitters: []
            };
            
            // Load emitters from skill or actor notetag
            var emitterIds = playerParams.emitterIds || notetag.emitter_ids;
            if (emitterIds && Array.isArray(emitterIds)) {
                // Expand emitter group IDs to actual emitter arrays
                var expandedIds = [];
                emitterIds.forEach(function(groupId) {
                    var expanded = my.expandEmitterGroup(groupId);
                    expandedIds = expandedIds.concat(expanded);
                });
                
                // Load each emitter by ID
                expandedIds.forEach(function(emitterId) {
                    var emitter = my.getEmitterById(emitterId);
                    if (emitter) {
                        playerData.emitters.push(emitter);
                    }
                });
            }
        } else {
            // Old system: Load from BulletHell.json
            playerData = $dataBulletHell.players[id];
        }
    ...
    BHell_Player.prototype.updateBonusLives = function () {
        // Skip bonus lives update if not in actual game (e.g., preview mode)
        if (!$gameBHellResult) {
            return;
        }
...
Scene_BHell_Init.prototype.createPlayersWindow = function() {
    ...
    for (var i = 0; i < $gamePlayer.bhellPlayers.length; i++) {
        if ($gamePlayer.bhellPlayers[i] && $gamePlayer.bhellPlayers[i].unlocked === true) {
...
Scene_BHell_Init.prototype.selectPlayer = function(i) {
    ...
    // Play select sound effect from actor notetag if available
    var playerData = $gamePlayer.bhellPlayers[i];
    if (playerData && playerData.notetag && playerData.notetag.select_se) {
        AudioManager.playSe(playerData.notetag.select_se);
    } else if ($dataBulletHell && $dataBulletHell.players && $dataBulletHell.players[i] && $dataBulletHell.players[i].select_se != null) {
...
var BHell = (function (my) {
    ...
    Scene_BHell.prototype.updateHUD = function () {
        ...
        var player = $gamePlayer.bhellPlayers[my.playerId];
        ...
        var bombIconIndex = $dataSkills[player.bombSkillId].iconIndex;
        sx = this.bomb.width / 16 * (bombIconIndex % 16);
        sy = this.bomb.height / 20 * Math.floor(bombIconIndex / 16);
    ...
    Scene_BHell.prototype.createHUD = function () {
        ...
        this.bomb = ImageManager.loadSystem("IconSet", 0);
    ...
    Scene_BHell.prototype.createWindows = function () {
        ...
        this.messageWindow.subWindows().forEach(function (window) {
            ...
            // Override updatePlacement for each sub-window to account for screen offset
            if (window.updatePlacement) {
                var originalSubPlacement = window.updatePlacement.bind(window);
                window.updatePlacement = function() {
                    originalSubPlacement();
        ...
        // Override updatePlacement to account for screen offset
        // Note: Window_Message already centers itself to boxWidth, so we need to recalculate
        var originalUpdatePlacement = this.messageWindow.updatePlacement.bind(this.messageWindow);
        this.messageWindow.updatePlacement = function() {
            originalUpdatePlacement();
            var offsetX = (Graphics.width - Graphics.boxWidth) / 2;
            var offsetY = (Graphics.height - Graphics.boxHeight) / 2;
            // Window is already centered to boxWidth, just add offsetX
            this.x = offsetX + (Graphics.boxWidth - this.width) / 2;
            this.y = offsetY + (this._positionType * (Graphics.boxHeight - this.height)) / 2;
        };
...
var BHell = (function (my) {
    ...
    Scene_BHell_Shop.prototype.cacheSprites = function () {
        ...
        ImageManager.loadSystem("IconSet", 0);
    ...
    Scene_BHell_Shop.prototype.commandBuyUpgrades = function () {
        var hasUnlockedPlayers = $gamePlayer.bhellPlayers && $gamePlayer.bhellPlayers.some(p => p && p.unlocked);
        
        if (!hasUnlockedPlayers) {
    ...
    Scene_BHell_Shop.prototype.onSelectPlayerOk = function () {
        var item = this.selectPlayerWindow.item();
        if (!item) {
            SoundManager.playBuzzer();
            return;
        }
    ...
    Scene_BHell_Shop.prototype.onBuyUpgradesOk = function () {
        var item = this.buyUpgradesWindow.item();
        if (!item) {
            SoundManager.playBuzzer();
            return;
        }
    ...
    Scene_BHell_Shop.prototype.buyingUpgradePrice = function () {
        return this._item ? this.buyUpgradesWindow.price(this._item) : 0;
    };
...
var BHell = (function (my) {
    ...
    BHell_Window_Players.prototype.updatePlacement = function () {
        ...
        var offsetY = (Graphics.height - Graphics.boxHeight) / 2;
        this.x = offsetX + (Graphics.boxWidth - this.width) / 2;
        this.y = offsetY + Graphics.boxHeight - this.height;
    };

    BHell_Window_Players.prototype.makeCommandList = function () {
        for (var i = 0; i < $gamePlayer.bhellPlayers.length; i++) {
            if ($gamePlayer.bhellPlayers[i] && $gamePlayer.bhellPlayers[i].unlocked === true) {
                // Get actor name from database if using Actor system
                var playerName;
                if ($gamePlayer.bhellPlayers[i].actorId) {
                    var actor = $dataActors[$gamePlayer.bhellPlayers[i].actorId];
                    playerName = actor ? actor.name : "Player " + (i + 1);
                } else if ($dataBulletHell && $dataBulletHell.players && $dataBulletHell.players[i]) {
                    playerName = $dataBulletHell.players[i].name;
                } else {
                    playerName = "Player " + (i + 1);
                }
                this.addCommand(playerName, String(i), true);
            }
        }
    };
    ...
    BHell_Window_Status.prototype.updatePlacement = function () {
        var offsetX = (Graphics.width - Graphics.boxWidth) / 2;
        var offsetY = (Graphics.height - Graphics.boxHeight) / 2;
        this.x = offsetX + (Graphics.boxWidth - this.width) / 2 + this.windowWidth();
        this.y = offsetY + (Graphics.boxHeight - this.height) / 2;
    };
    ...
    BHell_Window_ShopCommand.prototype.makeCommandList = function () {
        var hasUnlockedPlayers = $gamePlayer.bhellPlayers && $gamePlayer.bhellPlayers.some(p => p && p.unlocked);
        this.addCommand(my.buyUpgrades, 'upgrades', hasUnlockedPlayers);
    ...
    BHell_Window_SelectPlayer.prototype.drawItem = function (index) {
        ...
        if (!item) {
            this.drawText("No players available", rect.x, rect.y, rect.width);
            return;
        }
        
        // Get player data for display
        var playerData = {};
        if (item.actorId && $dataActors[item.actorId]) {
            var actor = $dataActors[item.actorId];
            playerData.name = actor.name;
            playerData.iconIndex = actor.faceIndex;
        } else if ($dataBulletHell && $dataBulletHell.players && $dataBulletHell.players[item.index]) {
            playerData = $dataBulletHell.players[item.index];
        } else {
            playerData.name = "Player " + (item.index + 1);
            playerData.iconIndex = 0;
        }
        
        this.drawItemName(playerData, rect.x, rect.y, rect.width);
    };
    ...
    BHell_Window_BuyUpgrades.prototype.price = function (item) {
        if (!item || !this._data) return 0;
        var index = this._data.indexOf(item);
        return index >= 0 ? this._price[index] || 0 : 0;
    };
    ...
    BHell_Window_BuyUpgrades.prototype.drawItem = function (index) {
        ...
        if (!item) {
            ...
            this.drawText("No upgrades available", rect.x, rect.y, rect.width);
            return;
        }
    ...
    BHell_Window_BuyUpgrades.prototype.select = function (index) {
        ...
        if (this.statusWindow && index >= 0 && index < this.maxItems()) {
...
// Override Window_ShopStatus to display BulletHell player preview for items with bhell_actor notetag
var BHell = (function (my) {
    ...
    Window_ShopStatus.prototype.initialize = function(x, y, width, height) {
        ...
        this._bhellActorId = null;
        this._bhellPlayer = null;
        this._bhellPreviewCounter = 0;
        this._bhellPlayerIndex = -1;
        
        // Create sprite container for BulletHell player preview
        this._bhellSpriteContainer = new Sprite();
        this.addChild(this._bhellSpriteContainer);
    };

    var _Window_ShopStatus_setItem = Window_ShopStatus.prototype.setItem;
    Window_ShopStatus.prototype.setItem = function(item) {
        _Window_ShopStatus_setItem.call(this, item);
        this.refreshBHellPreview();
        
        // Always force refresh to update display (whether BHell item or not)
        this.refresh();
    };

    /**
     * Checks if the current item is a BulletHell player item and refreshes the preview accordingly.
     */
    Window_ShopStatus.prototype.refreshBHellPreview = function() {
        // Clear previous preview
        if (this._bhellPlayer) {
            if (this._bhellSpriteContainer) {
                this._bhellSpriteContainer.removeChildren();
            }
            this._bhellPlayer = null;
        }
        this._bhellActorId = null;
        ...
        // Clear bullets from previous preview
        my.friendlyBullets = [];
        my.enemyBullets = [];
        
        if (!this._item || !DataManager.isItem(this._item)) {
            return;
        }
        
        // Check if item has bhell_actor notetag
        var actorId = my.parseItemNotetag(this._item);
        if (actorId && $dataActors[actorId]) {
            this._bhellActorId = actorId;
            
            // Initialize bhellPlayers if not exists yet
            if (!$gamePlayer.bhellPlayers) {
                $gamePlayer.bhellPlayers = [];
            }
            
            // Find player index in bhellPlayers array
            for (var i = 0; i < $gamePlayer.bhellPlayers.length; i++) {
                if ($gamePlayer.bhellPlayers[i] && $gamePlayer.bhellPlayers[i].actorId === actorId) {
                    this._bhellPlayerIndex = i;
                    break;
                }
            }
            
            // If player not found in bhellPlayers but actor exists, try to initialize it
            if (this._bhellPlayerIndex === -1) {
                // Check if this actor has BHell data
                var actor = $dataActors[actorId];
                if (actor && actor.note && actor.note.indexOf("<bhell:") >= 0) {
                    // Try to find empty slot or add new slot
                    var emptyIndex = $gamePlayer.bhellPlayers.length;
                    this._bhellPlayerIndex = emptyIndex;
                    
                    // Initialize basic player data
                    var actorClass = $dataClasses[actor.classId];
                    var notetag = my.parseActorNotetag(actor);
                    
                    $gamePlayer.bhellPlayers[emptyIndex] = {};
                    $gamePlayer.bhellPlayers[emptyIndex].index = emptyIndex;
                    $gamePlayer.bhellPlayers[emptyIndex].actorId = actorId;
                    $gamePlayer.bhellPlayers[emptyIndex].unlocked = false; // Not unlocked yet, just for preview
                    
                    // Stats from Classes
                    $gamePlayer.bhellPlayers[emptyIndex].speed = my.statToRank(actorClass.params[6][1]);
                    $gamePlayer.bhellPlayers[emptyIndex].rate = my.statToRank(actorClass.params[7][1]);
                    $gamePlayer.bhellPlayers[emptyIndex].power = my.statToRank(actorClass.params[2][1]);
                    $gamePlayer.bhellPlayers[emptyIndex].bombs = my.statToRank(actorClass.params[4][1]);
                    
                    // Load emitters and bomb data
                    var bhellSkills = my.getActorBHellSkills(actorId);
                    if (bhellSkills.weapons.length > 0) {
                        var primaryWeapon = bhellSkills.weapons[0];
                        if (primaryWeapon.data.emitter_ids) {
                            $gamePlayer.bhellPlayers[emptyIndex].emitterIds = primaryWeapon.data.emitter_ids;
                            $gamePlayer.bhellPlayers[emptyIndex].weaponSkillId = primaryWeapon.skillId;
                        }
                    } else if (notetag.emitter_ids) {
                        $gamePlayer.bhellPlayers[emptyIndex].emitterIds = notetag.emitter_ids;
                    }
                    
                    if (bhellSkills.bombs.length > 0) {
                        var primaryBomb = bhellSkills.bombs[0];
                        if (primaryBomb.data.bomb) {
                            $gamePlayer.bhellPlayers[emptyIndex].bombData = primaryBomb.data.bomb;
                            $gamePlayer.bhellPlayers[emptyIndex].bombSkillId = primaryBomb.skillId;
                        }
                    } else if (notetag.bomb) {
                        $gamePlayer.bhellPlayers[emptyIndex].bombData = notetag.bomb;
                    }
                    
                    $gamePlayer.bhellPlayers[emptyIndex].notetag = notetag;
                }
            }
            
            this.createBHellPreview();
        }
    };

    /**
     * Creates a BulletHell player preview with active shooting and bomb demo.
     */
    Window_ShopStatus.prototype.createBHellPreview = function() {
        if (!this._bhellActorId || !this._bhellSpriteContainer) return;
        
        // Clear previous bullets
        my.friendlyBullets = [];
        my.enemyBullets = [];
        
        // Create player (like in BHell_Window_Preview)
        if (this._bhellPlayerIndex !== -1) {
            this._bhellPlayer = new my.BHell_Player(this._bhellPlayerIndex, 0, true, this._bhellSpriteContainer);
            this._bhellPlayer.justSpawned = false;
            this._bhellPlayer.opacity = 255;
            
            // Position player higher (in the upper third of the window, above stats)
            this._bhellPlayer.x = this.width / 2;
            this._bhellPlayer.y = this.height * 0.35; // Position at 35% height, leaving space for stats below
    ...
    Window_ShopStatus.prototype.refresh = function() {
        // If this is a BulletHell item, display preview and stats
        if (this._bhellActorId) {
            this.contents.clear();
            if (this._item) {
                var x = this.textPadding();
                this.drawPossession(x, 0);
                
                // Draw stats if player data exists
                var actor = $dataActors[this._bhellActorId];
                if (actor && this._bhellPlayerIndex !== -1 && $gamePlayer.bhellPlayers[this._bhellPlayerIndex]) {
                    var player = $gamePlayer.bhellPlayers[this._bhellPlayerIndex];
                    
                    // Start stats display at line 6 (higher position)
                    var y = this.lineHeight() * 6;
                    
                    // Speed stat
                    this.changeTextColor(this.systemColor());
                    this.drawText(my.speed + ":", x, y, this.contents.width / 2);
                    this.resetTextColor();
                    var speedColor = this.getStatColor(player.speed);
                    this.changeTextColor(speedColor);
                    this.drawText(player.speed, x + this.contents.width / 2, y, this.contents.width / 2);
                    
                    // Rate stat
                    y += this.lineHeight();
                    this.changeTextColor(this.systemColor());
                    this.drawText(my.rate + ":", x, y, this.contents.width / 2);
                    this.resetTextColor();
                    var rateColor = this.getStatColor(player.rate);
                    this.changeTextColor(rateColor);
                    this.drawText(player.rate, x + this.contents.width / 2, y, this.contents.width / 2);
                    
                    // Power stat
                    y += this.lineHeight();
                    this.changeTextColor(this.systemColor());
                    this.drawText(my.power + ":", x, y, this.contents.width / 2);
                    this.resetTextColor();
                    var powerColor = this.getStatColor(player.power);
                    this.changeTextColor(powerColor);
                    this.drawText(player.power, x + this.contents.width / 2, y, this.contents.width / 2);
                    
                    // Bombs stat
                    y += this.lineHeight();
                    this.changeTextColor(this.systemColor());
                    this.drawText(my.bombs + ":", x, y, this.contents.width / 2);
                    this.resetTextColor();
                    var bombsColor = this.getStatColor(player.bombs);
                    this.changeTextColor(bombsColor);
                    this.drawText(player.bombs, x + this.contents.width / 2, y, this.contents.width / 2);
    ...
    /**
     * Returns color based on stat rank.
     */
    Window_ShopStatus.prototype.getStatColor = function(rank) {
        switch (rank) {
            case "S":
                return this.textColor(17); // Gold
            case "A":
                return this.textColor(2);  // Orange
            case "B":
            case "C":
            case "D":
            default:
    ...
    Window_ShopStatus.prototype.update = function() {
        ...
        // Update preview like in BHell_Window_Preview
        if (this._bhellPlayer) {
            // Set my.player for bomb updates during sprite updates
            my.player = this._bhellPlayer;
            
            this._bhellPreviewCounter++;
            
            // Shoot and bomb demo
            if (!this._bhellPlayer.bomb.isActive()) {
                if (this._bhellPreviewCounter % 300 === 0) {
                    this._bhellPlayer.launchBomb();
                } else {
                    this._bhellPlayer.shoot(true);
                }
            } else {
                this._bhellPlayer.shoot(false);
            }
            
            // Clean up bullets that go outside the window
            for (var i = 0; i < my.friendlyBullets.length; i++) {
                var b = my.friendlyBullets[i];
                if (b.x < 0 || b.y < 0 || b.x > this.width || b.y > this.height) {
                    b.destroy();
                    i--;
                }
    ...
    // Override Window_ShopBuy to hide items that are already owned (for BulletHell items)
    var _Window_ShopBuy_makeItemList = Window_ShopBuy.prototype.makeItemList;
    Window_ShopBuy.prototype.makeItemList = function() {
        _Window_ShopBuy_makeItemList.call(this);
        
        // Filter out BulletHell items that are already owned
        var filteredData = [];
        var filteredPrice = [];
        
        for (var i = 0; i < this._data.length; i++) {
            var item = this._data[i];
            var actorId = my.parseItemNotetag(item);
            
            // If this is a BulletHell item and already owned, skip it
            if (actorId && DataManager.isItem(item) && $gameParty.hasItem(item)) {
                continue;
            }
    ...
    // Override Scene_Shop.onBuyOk to skip quantity selection for BulletHell items
    var _Scene_Shop_onBuyOk = Scene_Shop.prototype.onBuyOk;
    Scene_Shop.prototype.onBuyOk = function() {
        this._item = this._buyWindow.item();
        
        // Check if this is a BulletHell item
        var actorId = BHell.parseItemNotetag(this._item);
        
        if (actorId && DataManager.isItem(this._item)) {
            // Skip quantity window for BulletHell items
            this._buyWindow.hide();
            SoundManager.playShop();
            this.doBuy(1); // Always buy quantity 1
            this.endNumberInput();
            ...
            // Normal behavior for non-BulletHell items
