...
(function () {
    const DebugManager = {
        ...
        initialize: function () {
            ...
            document.addEventListener("keydown", (event) => {
                // Cek jika F2 ditekan (keyCode 113)
        ...
        cycleMode: function () {
            this.debugMode = (this.debugMode + 1) % 3; // Siklus 0 -> 1 -> 2 -> 0
            if (this.debugDiv) {
                this.debugDiv.style.display = this.debugMode > 0 ? "block" : "none";
            }
            if (this.debugMode > 0) {
                this.updateInfo(this.lastAudioInfo); // Tampilkan info terakhir saat mode diubah
            }
        },
        ...
        updateInfo: function (info) {
            ...
            let playbackMode = this.lastAudioInfo.playbackMode || "N/A"; // Ganti default menjadi "N/A"
            const loadTime =
            ...
            // Kapitalisasi hanya huruf pertama dari playbackMode
            if (playbackMode.length > 0) {
                playbackMode = playbackMode.charAt(0).toUpperCase() + playbackMode.slice(1);
            }

            // Buat konten HTML dengan format judul baru
            const content = `
    ...
    const BackendManager = {
        _state: {
            ...
            libopenmpt: "unloaded",
            libopenmpt_mode: "none", // Akan menjadi 'worklet' atau 'legacy'
        },
        _promises: {},
            ...
            return new Promise((resolve, reject) => {
                ...
                WebAudio._context.audioWorklet
                    ...
                    .catch(() => {
                        ...
                        // Upaya 2: Muat skrip legacy sebagai fallback
        ...
        _requireGeneric: function (name, path, readinessCheck) {
            return new Promise((resolve, reject) => {
                ...
                // Jangan reject jika sedang loading untuk fallback
                ...
                script.onerror = () => {
                    // Jangan set state failed di sini, biarkan pemanggil yang menangani
    ...
    ExternalAudio.prototype.initialize = function (url, format) {
        ...
        this._gainNode = null;
        this._workletNode = null; // <-- TAMBAHKAN BARIS INI
        this._load();
            ...
            BackendManager.require(backendName)
                .then(() => {
                    ...
                    if (this._format === "midi") {
                        ...
                        // Cek mode yang berhasil dimuat oleh BackendManager
                        if (BackendManager.getModMode() === "worklet") {
                            this._playModWorklet();
                        } else {
                            this._playMod(); // Fallback ke metode legacy
                        }
                    }
                })
                .catch((error) => {
    ...
    ExternalAudio.prototype._reportDebugInfo = function () {
        ...
        if (this._format === "mod") {
            // Ambil mode langsung dari BackendManager: 'worklet' atau 'legacy'
            mode = BackendManager.getModMode();
        } else if (this._format === "midi") {
            // PicoAudio menggunakan ScriptProcessorNode secara internal untuk kompatibilitas luas.
            mode = "legacy";
        }

        DebugManager.updateInfo({
            fileName: decodeURIComponent(fileName),
            backend: backendName, // Data baru: Hanya nama backend (mis: libopenmpt)
            playbackMode: mode, // Data baru: Mode spesifik (mis: worklet)
    ...
    ExternalAudio.prototype._load = function () {
        ...
        xhr.onload = () => {
            this._isLoading = false;
            if (xhr.status < 400) {
                this._loadTime = performance.now() - startTime; // HITUNG & SIMPAN WAKTU
    ...
    ExternalAudio.prototype.fadeOut = function (duration) {
        if (this._format === "midi" && this._pico) {
            this._manualFade(duration, false); // false berarti ini adalah fade out
    ...
    ExternalAudio.prototype.stop = function () {
        if (this._format === "midi") {
            ...
            // Hentikan kedua kemungkinan player untuk memastikan kebersihan
            this._stopMod(); // Hentikan player legacy
            this._stopModWorklet(); // Hentikan player worklet
        }
    };
    ...
    ExternalAudio.prototype._playMidi = function () {
        ...
        this._pico.play();
        this._reportDebugInfo(); // <-- TAMBAHKAN INI
    };
    ...
    ExternalAudio.prototype._playModWorklet = function () {
        ...
        // Menangani pesan dari worklet, seperti akhir lagu
        ...
        // Mengirim konfigurasi awal ke worklet
        const config = {
        ...
        // Mengirim buffer audio untuk dimainkan
        this._workletNode.port.postMessage({ cmd: "play", val: this._buffer });
        this._decodeTime = performance.now() - startTime; // Waktu untuk setup node

        this._setupModWebAudioNodes(); // Menyiapkan gain node
        this._workletNode.connect(this._gainNode); // Menghubungkan worklet ke output
