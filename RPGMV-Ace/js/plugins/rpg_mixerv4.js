...
ExternalAudio.prototype._playMidi = function (startTime, isResuming) {
    ...
    // KUNCI #1: Selalu jeda sequencer sebelum melakukan operasi apa pun.
    // Ini memastikan state selalu dapat diprediksi dan menghentikan
    // sequencer yang mungkin berjalan tanpa suara dari BGM sebelumnya.
    sequencer.pause();

    // Jika kita TIDAK melanjutkan, muat lagu baru
    if (!isResuming) {
        sequencer.loadNewSongList([{ binary: this._buffer }]);
        sequencer.loopCount = this._loop ? Infinity : 0;
    }

    // Terapkan volume (ini secara otomatis membatalkan mute dari _stopMidi)
    this.volume = this._volume;

    // Atur posisi dan mainkan
