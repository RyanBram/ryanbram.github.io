...
(function () {
    ...
    Window_Options.prototype.drawItem = function (index) {
        ...
        if (this.isCategorySymbol(symbol)) {
            ...
            // Draw underline for category
            const lineY = rect.y + rect.height - 4;
            this.contents.fillRect(rect.x, lineY, rect.width, 2, this.systemColor());
            this.resetTextColor(); // Reset color after drawing category
            return;
        }
        ...
        for (let i = 0; i < this._handleSprites.length; i++) {
            ...
            if (index < this.maxItems()) {
                ...
                if (isGaugeItem) {
                    ...
                    // [FIX] Handle position: keep left at gauge start, but clamp right
                    const handleOffset = HANDLE_WIDTH / 2;
                    const effectiveGaugeWidth = gaugeWidth - handleOffset;
                    const handleX = gaugeAreaX + effectiveGaugeWidth * rate;
    ...
    Window_Options.prototype.processTouch = function () {
        ...
        if (this._isDragging) {
            if (TouchInput.isMoved()) {
                ...
                // [FIX] Apply same handle offset as in updateHandles
                const touchX = this.canvasToLocalX(TouchInput.x);
                const handleOffset = HANDLE_WIDTH / 2;
                const effectiveGaugeWidth = gaugeWidth - handleOffset;
                const relativeX = (touchX - this.padding - gaugeAreaX).clamp(0, effectiveGaugeWidth);

                const rate = relativeX / effectiveGaugeWidth;
                let newValue;
