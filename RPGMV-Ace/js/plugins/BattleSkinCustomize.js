...
/*:.
 * @plugindesc Plugin to assist in designing your own battle screen.
 * @author Triacontan
 *
 * @param 位置調整を行う
 * @desc Adjust window position from battle test.
 * @drag & drop to adjust position.
 * @default ON
 *
 * @param ウィンドウ透過
 * @desc Display transparently when windows overlap.
 * Disable this function if other plug-ins realize the same function.
 * @default OFF
 *
 * @param グリッドサイズ
 * @desc Display grid with specified size during window adjustment.
 * If 0 is specified, it will be hidden.
 * @default 48
 *
 * @help Eliminate the conventional status window and
 * Allows you to freely design the content and position of the display of actor information.
 * Does not affect the combat system.
 *
 * Specifically, the following will be displayed in place of the traditional status window.
 * By adjusting the values defined in the user-defined area (settings) of the source code.
 * You can customize the position, appearance, and display content.
 *
 * ・Status display area
 * This area displays information for each individual actor participating in the battle.
 * It is always displayed during combat.
 * * Settings for this area are defined in [settings.actorStatus].
 *
 * * ・Area for displaying the currently selected actor.
 * This area displays information on the actor currently selected for a command.
 * This area is displayed only when a command is selected.
 * * Settings for this area are defined in [settings.selectedActor].
 *
 * Without being bound by the conventional concept of front view and side view, it is possible to
 * You can design your own battle screen by placing any parts you like anywhere you like.
 *
 * Attention!
 * All necessary images must be placed under “/img/pictures/” in png format.
 * Note that the extension is not necessary when specifying in the user-defined area.
 *
 * Plug-in command details
 * Executed from the event command “Plug-in command.
 * (Separate parameters with a single-byte space.)
 *
 * There is no plugin command in this plugin.
 *
 * Terms of use:
 * * * You can modify and redistribute this plugin without permission of the author, and there are no restrictions on the type of use (commercial, 18 prohibited use, etc.).
 * * There are no restrictions on the type of use (commercial, 18-content use, etc.).
...
(function () {
    ...
    var settings = {
        ...
        fonts: [
            ...
            { name: "Mplus", url: "fonts/mplus-1m-regular.ttf" },
        ],
        ...
        actorStatus: {
            ...
            items: {
                ...
                hp: {
                    ...
                    value: { visible: true, x: 36, y: 30, digit: 4, max: true, zeroPadding: false },
                    gauge: { visible: true, x: 168, y: 40, width: 100, height: 8 },
                },
                mp: {
                    label: { visible: true, x: 4, y: 62 },
                    value: { visible: true, x: 36, y: 62, digit: 4, max: true, zeroPadding: false },
                    gauge: { visible: true, x: 168, y: 72, width: 100, height: 8 },
                },
                tp: {
                    label: { visible: true, x: 4, y: 94 },
                    value: { visible: true, x: 36, y: 94, digit: 4, max: true, zeroPadding: false },
    ...
    Scene_Boot.prototype.isGameFontLoaded = function () {
        ...
        var result = settings.fonts.every(
            function (fontInfo) {
    ...
    if (Utils.isOptionValid("btest") && Utils.isNwjs()) {
        ...
        Window_Base.prototype.isTouchPosInRect = function () {
            ...
            var tx = TouchInput.x;
            var ty = TouchInput.y;
            return tx >= this.x && tx <= endX && ty >= this.y && ty <= endY;
        };
