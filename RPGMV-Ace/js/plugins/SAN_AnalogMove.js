 ...
 * ■更新履歴
 * v3.2.0 (2025-01-21)
 * - FIXED: Direction priority - maintains last cardinal direction during diagonal
 * - 斜め移動時に最後のcardinal direction(4方向)を維持
 * - 挙動が一貫性のあるものになりました
 *
 * v3.1.5
 * - Previous release
...
Sanshiro.AnalogMove.version = "3.2.0";
...
(function () {
  ...
  Vector.prototype.equals = function (vec) {
    return this.len() === vec.len() && this.dir() === vec.dir();
  };
  ...
  CharacterMover.prototype.collidingCharacters = function () {
    var characters = [];
    if (!this.isThrough()) {
      characters = $gameMap.allCharacters().filter(function (character) {
  ...
  PlayerMover.prototype.collidingCharacters = function () {
    ...
    var events = characters.filter(function (character) {
  ...
  PlayerMover.prototype.isCollidableCharacter = function (character) {
    return (
      this.isCollidableEvent(character) || this.isCollidableVehicle(character)
    );
  };

  // 衝突し得るイベント
  PlayerMover.prototype.isCollidableEvent = function (character) {
    return character instanceof Game_Event && character.isNormalPriority();
  };
  ...
  FollowerMover.prototype.dpf = function () {
    var dpfCoef = $gamePlayer.areFollowersGathering()
      ? 1.0
      : !this._tarPosVec
        ? 1.0
  ...
  FollowerMover.prototype.updateThrough = function () {
    if ($gamePlayer.isThrough()) {
      ...
      this._through = this._tarPosVec.sub2(this._posVec).len() > 2.0;
    }
  };
  ...
  FollowerMover.prototype.updateTargetPosition = function () {
    ...
    this._tarPosVec = $gamePlayer.areFollowersGathering()
      ? preChaPosVec
      : relPosVec.len() > 1.0
        ? preChaPosVec.sub2(relPosVec.scl(1.0))
        : null;
  };
  ...
  Game_CharacterBase.prototype.setDirVec = function (dirVec) {
    // Handle empty/zero vector
    if (!dirVec || dirVec.len() === 0) {
      this.setDirection(CharacterMover.radToDir4(dirVec.dir()));
      return;
    }

    // Convert to 8-directional value
    var dir8 = CharacterMover.radToDir8(dirVec.dir());
    var oldDirection = this._direction;

    // Check if this is diagonal (1, 3, 7, 9)
    var isDiagonal = [1, 3, 7, 9].indexOf(dir8) !== -1;

    if (isDiagonal) {
      // Diagonal movement - maintain last cardinal direction
      // Mapping diagonal to cardinal components
      var diagonalToCardinals = {
        1: [4, 2], // Down-Left = Left or Down
        3: [6, 2], // Down-Right = Right or Down
        7: [4, 8], // Up-Left = Left or Up
        9: [6, 8], // Up-Right = Right or Up
      };

      var cardinals = diagonalToCardinals[dir8];

      // Check if last direction is still valid for this diagonal
      if (cardinals && cardinals.indexOf(oldDirection) !== -1) {
        // Maintain last cardinal direction
        this._direction = oldDirection;
      } else {
        // Last direction not valid, choose horizontal priority (more natural)
        // Horizontal: 4 (left) or 6 (right)
        // Vertical: 2 (down) or 8 (up)
        var horizontalDir = null;
        for (var i = 0; i < cardinals.length; i++) {
          if (cardinals[i] === 4 || cardinals[i] === 6) {
            horizontalDir = cardinals[i];
            break;
          }
        }
      ...
      // Cardinal direction (2, 4, 6, 8) or using radToDir4 as fallback
      var isCardinal = [2, 4, 6, 8].indexOf(dir8) !== -1;
      if (isCardinal) {
        this._direction = dir8;
      } else {
        // Fallback to original radToDir4 for any edge case
        this.setDirection(CharacterMover.radToDir4(dirVec.dir()));
      }
    }
  };
  ...
  Game_CharacterBase.prototype.isEdge = function (x, y, d) {
    ...
    if ($gameMap.isValid(x2, y2)) {
      if (!this.isThrough()) {
        return !this.isMapPassable(x, y, d) || !this.isMapPassable(x2, y2, d2);
      }
      return false;
    }
    return true;
  ...
  Game_CharacterBase.prototype.edgePos = function (x, y, d) {
    if (this.isEdge(x, y, d)) {
      return d === 6
        ? Vector.rect(x + 1.0, y + 0.5)
        : d === 2
          ? Vector.rect(x + 0.5, y + 1.0)
          : d === 4
            ? Vector.rect(x, y + 0.5)
            : d === 8
              ? Vector.rect(x + 0.5, y)
  ...
  // すり抜け判定
  var _Game_Player_isThrough = Game_Player.prototype.isThrough;
  ...
  Game_Player.prototype.shouleMoveDefault = function () {
    return (
      this._moveDefault || this._vehicleGettingOn || this._vehicleGettingOff
    );
  };
  ...
  Game_Player.prototype.updateIgnoringEventIds = function () {
    this._ignoringEventIds = this._ignoringEventIds.filter(function (eventId) {
  ...
  Game_Player.prototype.checkMoveDefault = function () {
    if (
      this.isMoveRouteCommand(this.nextRouteCommand()) ||
  ...
  Game_Follower.prototype.isThrough = function () {
    if (this.canAnalogMove()) {
      return !this.isVisible() || $gamePlayer.isThrough();
    }
  ...
  // 移動中判定
  var _Game_Follower_isMoving = Game_Follower.prototype.isMoving;
  ...
  // 移動の更新
  var _Game_Follower_updateMove = Game_Follower.prototype.updateMove;
  ...
  Game_Follower.prototype.canAnalogMove = function () {
    return (
      this.hasMover() &&
      !this.shouleMoveDefault() &&
      ($gamePlayer.canAnalogMove() || $gamePlayer.areFollowersGathering())
    );
  };
  ...
  Game_Event.prototype.shouldIgnore = function () {
    return this.isCollideTriggerEvent() && $gamePlayer.posNt(this._x, this._y);
  };
