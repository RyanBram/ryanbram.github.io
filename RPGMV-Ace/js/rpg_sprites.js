...
Sprite_Actor.MOTIONS = {
    wait: { index: 0, loop: true }, // edit
    walk: { index: 1, loop: true }, // edit
...
Sprite_Actor.prototype.initMembers = function () {
    ...
    this._motionCount = 0;
    this._pattern = 0;
    this._shakeDuration = 0; // <-- addition
...
Sprite_Actor.prototype.update = function () {
    ...
    if (this._actor) {
        this.updateMotion();
        this.updateShake(); // <-- addition
    }
};

Sprite_Actor.prototype.updateShake = function () {
    if (this._shakeDuration > 0) {
        this._shakeDuration--;
        var shakePower = 3; // shake power (eg: 3 piksel)
        if (this._shakeDuration > 0) {
            // Vibrating x to left and right relative to current position
            this.x = this._homeX + (this._shakeDuration % 2 === 0 ? shakePower : -shakePower);
        } else {
            // Returning x position after shaking
            this.x = this._homeX;
        }
    }
};
...
Sprite_Actor.prototype.setupMotion = function () {
    if (this._actor.isMotionRequested()) {
        var motionType = this._actor.motionType();
        if (motionType === "damage" && this._actor.isGuard()) {
            this._shakeDuration = 20; // shake duration (eg: 20 frames)
        } else {
...
Sprite_Actor.prototype.setupWeaponAnimation = function () {
    if (this._actor.isWeaponAnimationRequested()) {
        // Jika nama battler diawali dengan $, tidak perlu menampilkan weapon sprite
        var battlerName = this._actor.battlerName();
        if (battlerName && battlerName.charAt(0) === "$") {
            // Skip weapon animation untuk custom battler dengan senjata built-in
...
Spriteset_Base.prototype.initialize = function () {
    Sprite.prototype.initialize.call(this);
    this.filters = [new PIXI.AlphaFilter()]; // for picture
