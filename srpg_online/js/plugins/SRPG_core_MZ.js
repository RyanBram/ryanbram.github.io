...
(() => {
    ...
    var _srpgMenuCommandList =
        parameters["srpgMenuCommandList"] || "turnEnd,autoBattle,item,skill,equip,status,WinLose,options,save,gameEnd";
    ...
    Game_System.prototype.setSrpgActors = function () {
        ...
        if (_srpgNotShowUpDeadActor === "false") var array = $gameParty.allMembers();
            ...
            var array = $gameParty.allMembers().filter(function (actor) {
        ...
        $gameMap.events().forEach(function (event) {
            if (event.isType() === "actor" && !event.isErased()) {
                ...
                if (
                    _maxActorVarID > 0 &&
                    $gameVariables.value(_maxActorVarID) > 0 &&
    ...
    Game_System.prototype.unitDistance = function (event1, event2) {
        ...
        if ($gameMap.isLoopHorizontal() === true) {
            var event1X =
                event1.posX() > event2.posX() ? event1.posX() - $gameMap.width() : event1.posX() + $gameMap.width();
            var disX = Math.abs(event1X - event2.posX());
            minDisX = minDisX < disX ? minDisX : disX;
        }
        if ($gameMap.isLoopVertical() === true) {
            var event1Y =
                event1.posY() > event2.posY() ? event1.posY() - $gameMap.height() : event1.posY() + $gameMap.height();
    ...
    Game_System.prototype.setTargetDirection = function (targetEvent) {
        ...
        if ($gameMap.isLoopHorizontal() === true) {
            var event1X =
                $gameTemp.activeEvent().posX() > targetEvent.posX()
                    ? targetEvent.posX() - $gameMap.width()
                    : $gameTemp.activeEvent().posX() + $gameMap.width();
            var disX = event1X - targetEvent.posX();
            differenceX = Math.abs(differenceX) < Math.abs(disX) ? differenceX : disX;
        }
        if ($gameMap.isLoopVertical() === true) {
            var event1Y =
                $gameTemp.activeEvent().posY() > targetEvent.posY()
                    ? $gameTemp.activeEvent().posY() - $gameMap.height()
                    : $gameTemp.activeEvent().posY() + $gameMap.height();
    ...
    Game_System.prototype.areTheyNoUnits = function (x, y, type) {
        ...
        $gameMap.eventsXy(x, y).forEach(function (event) {
            var battlerArray = $gameSystem.EventToUnit(event.eventId());
            if (
                (battlerArray && event !== $gameTemp.activeEvent() && !event.isErased()) ||
    ...
    Game_System.prototype.isThereEventUnit = function (x, y, type) {
        ...
        if (type === "actor") {
            $gameMap.eventsXy(x, y).forEach(function (event) {
                if (
                    event.isType() === "unitEvent" ||
                    event.isType() === "unitEventForActor" ||
    ...
    Game_System.prototype.srpgMakeMoveTableOriginalPos = function (event) {
        ...
        event.makeMoveTable(
            $gameTemp.originalPos()[0],
            $gameTemp.originalPos()[1],
            $gameTemp.originalMove(),
            null,
            user.srpgThroughTag()
        );
        if (item.meta.notUseAfterMove) {
            // cannot move before attacking
            var x = event.posX();
            var y = event.posY();
    ...
    Game_System.prototype.isCancelButtonEnabled = function () {
        return (
            this.isSubBattlePhase() === "actor_move" ||
    ...
    Game_BattlerBase.prototype.makeAllRangeListForBattle = function (skill, battler) {
        ...
        $gameMap.events().forEach(function (event) {
            if (
                (specialRange === "allActor" && event.isType() === "actor" && !event.isErased()) ||
    ...
    Game_BattlerBase.prototype.canInput = function () {
        if ($gameSystem.isSRPGMode()) {
            return this.isAppeared() && !this.isRestricted() && !this.isAutoBattle() && !this.srpgTurnEnd();
        } else {
    ...
    Game_BattlerBase.prototype.canUse = function (item) {
        if ($gameSystem.isSRPGMode()) {
            ...
            if (this._srpgActionTiming !== 1 && Number(item.meta.srpgAreaRange) > 0) {
                ...
                if (
                    !$gameSystem.srpgAIUnderstandsAoE &&
    ...
    Game_Battler.prototype.useItem = function (item) {
        if ($gameSystem.isSRPGMode()) {
            if (DataManager.isSkill(item)) {
                if (!this._usedFirstSkill || _srpgAgiAttackPlusPayCost === 2 || _srpgAgiAttackPlusPayCost === 4) {
                    this.paySkillCost(item);
                    this._usedFirstSkill = true;
                }
            } else if (DataManager.isItem(item)) {
                if (!this._usedFirstSkill || _srpgAgiAttackPlusPayCost === 3 || _srpgAgiAttackPlusPayCost === 4) {
    ...
    Game_Actor.prototype.srpgReactionSkill = function () {
        ...
        // 職業
        if (this.currentClass().meta.srpgReactionSkill)
            array.unshift(Number(this.currentClass().meta.srpgReactionSkill));
    ...
    Game_Actor.prototype.srpgAlternativeSkillId = function () {
        const id = this.actor().meta.srpgAlternativeSkillId
            ? Number(this.actor().meta.srpgAlternativeSkillId)
            : this.attackSkillId();
        return id;
    };
    ...
    Game_Actor.prototype.canSrpgEquip = function () {
        return (
            $gameTemp.originalPos()[0] === $gameTemp.activeEvent().posX() &&
    ...
    Game_Enemy.prototype.srpgAlternativeSkillId = function () {
        const id = this.enemy().meta.srpgAlternativeSkillId
            ? Number(this.enemy().meta.srpgAlternativeSkillId)
            : this.attackSkillId();
        return id;
    };
    ...
    Game_Party.prototype.members = function () {
        if ($gameSystem.isSRPGMode()) {
            if (
                $gameSystem.isSubBattlePhase() === "initialize" ||
    ...
    Game_Troop.prototype.setup = function (troopId) {
        if ($gameSystem.isSRPGMode()) {
            ...
            for (var i = 0; i < this.SrpgBattleEnemys().length; i++) {
                ...
                if ($gameSystem.isSideView()) {
                    enemy.setScreenXy(
                        Graphics.boxWidth / 4 + 240 * i + enemy.correctionX(),
                        Graphics.boxHeight / 2 + 48 + enemy.correctionY()
                    );
                } else {
                    enemy.setScreenXy(
                        Graphics.boxWidth / 2 + enemy.correctionX(),
                        Graphics.boxHeight / 2 + 32 + 96 * i + enemy.correctionY()
                    );
                }
                this._enemies.push(enemy);
            }
    ...
    Game_CharacterBase.prototype.isSrpgCollidedWithEvents = function (x, y) {
        ...
        return events.some(function (event) {
            if (
                (event.isType() === "actor" && $gameTemp.activeEvent().isType() === "enemy") ||
    ...
    Game_CharacterBase.prototype.srpgMoveCanPass = function (x, y, d, tag) {
        ...
        if (
            ($gameMap.terrainTag(x2, y2) > 0 && $gameMap.terrainTag(x2, y2) <= tag) ||
            ($gameMap.terrainTag(x, y) > 0 &&
                $gameMap.terrainTag(x, y) <= tag &&
        ...
        for (let i = 0; i < edges.length; i++) {
            ...
            for (let num = 0; num < events.length; num++) {
                ...
                if (event.isType() === "actor" || event.isType() === "enemy") {
                    ...
                    (event.isType() === "object" && event.characterName() !== "") ||
                    event.isType() === "playerEvent"
                ) {
                    noEvents = false;
                }
    ...
    Game_Player.prototype.moveByInput = function () {
        if ($gameSystem.isSRPGMode() && $gameTemp.isAutoMoveDestinationValid() === true && !this.isMoving()) {
            // 移動先と現在地の関係を計算する
            ...
            if ($gameMap.isLoopHorizontal() === true) {
                ...
                var destX =
                    $gameTemp.autoMoveDestinationX() > this.x
                        ? $gameTemp.autoMoveDestinationX() - $gameMap.width()
                        : $gameTemp.autoMoveDestinationX() + $gameMap.width();
            ...
            if ($gameMap.isLoopVertical() === true) {
                ...
                var destY =
                    $gameTemp.autoMoveDestinationY() > this.y
                        ? $gameTemp.autoMoveDestinationY() - $gameMap.height()
                        : $gameTemp.autoMoveDestinationY() + $gameMap.height();
    ...
    Game_Player.prototype.inCancelButtonArea = function () {
        ...
        return TouchInput.x > left && TouchInput.x < right && TouchInput.y > upper && TouchInput.y < lower;
    };
                    ...
                    $gameMap.eventsXy(x, y).forEach(function (event) {
                        if (event.isTriggerIn(triggers) && !event.isErased()) {
                            ...
                            if (event.isType() === "actor" || event.isType() === "enemy") {
                                ...
                                var target = targetArray[1];
                                if (
                                    (targetArray[0] === "actor" && user.currentAction().isForFriend()) ||
    ...
    Game_Interpreter.prototype.checkUserAndTarget = function (switchId, eventId1, eventId2) {
        if ($gameTemp.activeEvent() && $gameTemp.targetEvent()) {
            if (
                ($gameTemp.activeEvent().eventId() === eventId1 && $gameTemp.targetEvent().eventId() === eventId2) ||
    ...
    Game_Interpreter.prototype.checkActiveOrTarget = function (switchId) {
        ...
        const y = eventUnit.y;
        $gameMap.eventsXy(x, y).forEach(function (event) {
            if (!event.isErased() && (event.isType() === "actor" || event.isType() === "enemy")) {
            ...
            if (battler) {
                ...
                if (battler.isAlive()) {
                    if (battler.isRestricted()) {
                        ...
                        this.createTurnEndSprites();
                        this.turnEndSpritesSetBitmap(sx, sy, pw, ph);
                    } else if (battler.isActor() === true && battler.isAutoBattle()) {
                        // 自動戦闘の場合
    ...
    Spriteset_Map.prototype.update = function () {
        ...
        if (
            $gameSystem.isSRPGMode() ||
            $gameTemp.resetMoveList() === true ||
            $gameTemp.isMoveListValid() ||
            this._srpgMoveTile[0].isThisMoveTileValid()
        ) {
    ...
    Window_SrpgBattleResult.prototype.drawGainExp = function (x, y) {
        ...
        if (nowExp >= this._battler.expForLevel(this._level + 1) && this._level < this._battler.maxLevel()) {
            this._level += 1;
            const se = {};
            se.name = _expSe;
            se.pan = 0;
        ...
        if (this._level >= this._battler.maxLevel()) {
            ...
            rate =
                (nowExp - this._battler.expForLevel(this._level)) /
    ...
    Window_SrpgPrediction.prototype.drawSrpgBattleOrders = function (battler, item, x, y) {
        ...
        if (DataManager.isSkill(item) && item.id === battler.attackSkillId() && !battler.hasNoWeapons()) {
            if (battler.isActor()) {
    ...
    */
    ...
    BattleManager.endTurn = function () {
        if ($gameSystem.isSRPGMode()) {
            ...
            //this.refreshStatus();
            if (this._phase) {
                if ($gameParty.battleMembers().length > 0 && !$gameParty.isAllDead()) {
                    // edit so gain exp when there are live members
    ...
    BattleManager.shareExp = function () {
        return (
            $gameSystem.isSRPGMode() == true && $gameSystem.EventToUnit($gameTemp.activeEvent().eventId())[0] != "actor"
        );
    };
    ...
    Scene_Map.prototype.startActorTargetting = function () {
        ...
        if (!action.isForDeadFriend()) {
            ...
            event.makeRangeTable(
                event.posX(),
                event.posY(),
                battler.srpgSkillRange(item),
                [0],
                event.posX(),
                event.posY(),
                item
    ...
    Scene_Map.prototype.onDeadActorOk = function () {
        if ($gameParty.targetActor().isDead()) {
            ...
            var skill = user.currentAction().item();
            $gameTemp.clearMoveTable();
            event1.makeRangeTable(
                event1.posX(),
                event1.posY(),
                user.srpgSkillRange(skill),
                [0],
                event1.posX(),
                event1.posY(),
                skill
            ...
            var xy = event2.makeAppearPoint(
                event2,
                event2.posX(),
                event2.posY(),
                targetBattlerArray[1].srpgThroughTag()
            );
    ...
    Scene_Map.prototype.selectPreviousSrpgBattleStart = function () {
        ...
        if (activeBattlerAction) {
            if (
                (_srpgSkipTargetForSelf === "true" && activeBattlerAction.isForUser()) ||
    ...
    Scene_Map.prototype.srpgInvokeAutoActorMove = function () {
        ...
        // 行動対象としうるユニットがいる場合
        if (
            canAttackTargets.length > 0 ||
            actor.battleMode() === "absRegionUp" ||
            actor.battleMode() === "absRegionDown" ||
            actor.battleMode() === "regionUp" ||
            actor.battleMode() === "regionDown"
        ) {
            // ターゲットの設定
    ...
    Scene_Map.prototype.srpgInvokeEnemyMove = function () {
        ...
        if (
            canAttackTargets.length > 0 ||
            enemy.battleMode() === "absRegionUp" ||
            enemy.battleMode() === "absRegionDown" ||
            enemy.battleMode() === "regionUp" ||
            enemy.battleMode() === "regionDown"
        ) {
            // ターゲットの設定
            var targetEvent = this.srpgDecideTarget(canAttackTargets, event, targetType);
            $gameTemp.setTargetEvent(targetEvent);
    ...
    Scene_Map.prototype.checkAlternativeSkill = function (event, type, battler, targetType, canAttackTargets) {
        ...
        if (
            (action.isHpRecover() && canAttackTargets.length === 0) ||
    ...
    Scene_Map.prototype.srpgSearchOptimalPos = function (targetEvent, battler, type) {
        ...
        if (
            $gameTemp.activeEvent() === targetEvent ||
            !targetEvent ||
            battler.battleMode() === "absRegionUp" ||
            battler.battleMode() === "absRegionDown" ||
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            if (pos[2] === false && $gameSystem.areTheyNoUnits(pos[0], pos[1])) {
                ...
                var check = range - dis;
                var specialRange = $gameTemp
                    .activeEvent()
                    .srpgRangeExtention(targetEvent.posX(), targetEvent.posY(), pos[0], pos[1], skill, range);
    ...
    Scene_Map.prototype.srpgSearchItemPos = function (list, candidatePos, type) {
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            if (pos[2] === false && $gameSystem.areTheyNoUnits(pos[0], pos[1])) {
                if (
                    $gameSystem.isThereEventUnit(pos[0], pos[1], type) &&
                    $gameSystem.indexOfSearchedItemList([pos[0], pos[1]]) < 0
                ) {
                    candidatePos = [];
                    candidatePos.push([pos[0], pos[1]]);
                }
    ...
    Scene_Map.prototype.preBattleSetDirection = function () {
        ...
        if ($gameMap.isLoopHorizontal() === true) {
            var event1X =
                $gameTemp.activeEvent().posX() > targetPosX
                    ? $gameTemp.activeEvent().posX() - $gameMap.width()
                    : $gameTemp.activeEvent().posX() + $gameMap.width();
        ...
        if ($gameMap.isLoopVertical() === true) {
            var event1Y =
                $gameTemp.activeEvent().posY() > targetPosY
                    ? $gameTemp.activeEvent().posY() - $gameMap.height()
                    : $gameTemp.activeEvent().posY() + $gameMap.height();
    ...
    Scene_Map.prototype.update = function () {
        ...
        if (!this.waitingForSkill() && !this._srpgBattleResultWindow.isChangeExp()) {
            ...
            if (
                !this.srpgHasMapSkills() &&
                !this._srpgBattleResultWindow.isOpen() &&
    ...
    Scene_Map.prototype.waitingForSkill = function () {
        ...
        if ($gamePlayer.isAnimationPlaying() || !$gamePlayer.isStopping() || $gameTemp.isAutoMoveDestinationValid())
            return true;
    ...
    Scene_Map.prototype.srpgInvokeMapSkill = function (data) {
        ...
        switch (data.phase) {
            ...
            case "start":
                if (
                    !user.canMove() ||
                    !user.canUse(action.item()) ||
                    (!action.isForDeadFriend() && !target.isAlive())
                ) {
                    data.phase = "cancel";
                ...
                if (!$gameTemp.isFirstAction || $gameTemp.isFirstAction(action)) {
                    ...
                    if (action.item().meta.directionalAnimation) {
                        ...
                        $gameTemp.requestAnimation(
                            [$gamePlayer],
                            dir + Number(action.item().meta.directionalAnimation)
                        );
                        castAnim = true;
                    }
                }
